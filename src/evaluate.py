...
def metrics(logits: torch.Tensor, targets: torch.Tensor, threshold: float = 0.5):
    """Return a range of metric values (not used for loss)."""
...
@torch.no_grad()
def evaluate(model, loader, device, threshold=0.5, desc: Optional[str] = None):
    """Evaluate the model from the evlaluation dataloader."""
