...
def build_track_npz(track,out_path: Path,sr: int,win_sec: float,hop_sec: float,n_fft: int,mel_hop: int,n_mels: int,ratio_thr_map: dict[str, float],min_mix_rms: float,stem_abs_thr: float,compress: bool) -> dict:
    ...
    payload = dict(X=X,Y=Y,T=T,sr=sr,win_sec=win_sec,hop_sec=hop_sec,
        n_fft=n_fft,mel_hop=mel_hop,n_mels=n_mels,min_mix_rms=min_mix_rms,
        stem_abs_thr=stem_abs_thr,ratio_thr_vocals=ratio_thr_map["vocals"],
        ratio_thr_drums=ratio_thr_map["drums"],ratio_thr_bass=ratio_thr_map["bass"],track_name=str(track.name))
    if compress:
        np.savez_compressed(out_path, **payload)
    else:
        np.savez(out_path, **payload)
    return {"saved": True,"total_windows": total_windows,"kept_windows": kept_windows,
        "skipped_silent": skipped_silent,"X_shape": tuple(X.shape),"Y_shape": tuple(Y.shape)}
...
def main():
    ...
    for subset in ["train", "test"]:
        ...
        for track in tqdm(db, desc=f"Processing {subset}", unit="track"):
            ...
            stats = build_track_npz(track=track,out_path=out_path,sr=args.sr,win_sec=args.win_sec,
                hop_sec=args.hop_sec,
                n_fft=args.n_fft,mel_hop=args.mel_hop,n_mels=args.n_mels,
                ratio_thr_map=ratio_thr_map,
                min_mix_rms=args.min_mix_rms,
                stem_abs_thr=args.stem_abs_thr,compress=compress)
            if stats["saved"]:
                saved += 1
