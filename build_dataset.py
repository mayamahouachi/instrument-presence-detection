...
Output per track:
  ...
  - Y: (N, 3) int8                  -> [vocals, drums, bass] presence labels per window
...
python build_dataset.py \
  ...
  --ratio-thr-vocals 0.2 \
  --ratio-thr-drums 0.25 \
  --ratio-thr-bass 0.3 \
  --min-mix-rms 1e-4 \
  --stem-abs-thr 0.002 \
  --max-track 2
...
def to_mono(x: np.ndarray) -> np.ndarray:
    ...
    if x.shape[0] <= 4 and x.shape[1] > 1000:
        x = x.T
    # moyenne des canaux
...
def logmel(y: np.ndarray, sr: int, n_fft: int, hop_length: int, n_mels: int) -> np.ndarray:
    ...
    return librosa.power_to_db(S, ref=np.max).astype(np.float32)  # (n_mels, frames)


def build_track_npz(track,out_path: Path,sr: int,win_sec: float,hop_sec: float,n_fft: int,mel_hop: int,n_mels: int,ratio_thr_map: dict[str, float],
                    min_mix_rms: float,stem_abs_thr: float, compress: bool) -> dict:
    ...
    max_start = len(mix) - win
    if max_start < 0:
        return {"saved": False, "total_windows": 0, "kept_windows": 0, "skipped_silent": 0}
    ...
    if kept_windows == 0:
        return { "saved": False,"total_windows": total_windows, "kept_windows": 0,"skipped_silent": skipped_silent}

    X = np.stack(X_list, axis=0).astype(np.float32)       # (N, n_mels, frames)
    Y = np.array(Y_list, dtype=np.int8)                   # (N, 3)
    T = np.array(T_list, dtype=np.float32)                # (N,)

    out_path.parent.mkdir(parents=True, exist_ok=True)
    payload = dict(X=X,Y=Y,T=T,sr=sr,win_sec=win_sec,hop_sec=hop_sec,n_fft=n_fft,mel_hop=mel_hop,n_mels=n_mels,min_mix_rms=min_mix_rms,stem_abs_thr=stem_abs_thr,
                   ratio_thr_vocals=ratio_thr_map["vocals"],ratio_thr_drums=ratio_thr_map["drums"],ratio_thr_bass=ratio_thr_map["bass"],track_name=str(track.name))
    ...
    return {"saved": True,"total_windows": total_windows,"kept_windows": kept_windows,"skipped_silent": skipped_silent,"X_shape": tuple(X.shape),"Y_shape": tuple(Y.shape)}
...
def sanity_report(npz_files: list[Path], max_files: int = 50) -> None:
    ...
    counts = np.zeros(len(STEMS), dtype=np.int64)
    total = 0
...
def main():
    ...
    p.add_argument("--min-mix-rms",type=float,default=1e-4,help="Skip silent windows based on mix RMS")
    p.add_argument( "--stem-abs-thr",type=float,default=1e-4,help="Absolute RMS gate for stems")
    p.add_argument( "--no-compress",action="store_true",help="Save as np.savez (uncompressed) instead of np.savez_compressed")
    p.add_argument("--max-track",type=int,default=None,help="Maximum number of tracks to process per subset for debugging")
    args = p.parse_args()
    ratio_thr_map = { "vocals": args.ratio_thr if args.ratio_thr_vocals is None else args.ratio_thr_vocals, "drums": args.ratio_thr if args.ratio_thr_drums is None else args.ratio_thr_drums,"bass": args.ratio_thr if args.ratio_thr_bass is None else args.ratio_thr_bass }
    ...
    print(f"  save={'np.savez_compressed' if compress else 'np.savez (uncompressed)'}")
    if args.max_track is not None:
        print(f"  max_track={args.max_track}")

    for subset in ["train", "test"]:
        subset_out = out_dir / subset
        subset_out.mkdir(parents=True, exist_ok=True)
        print(f"Loading MUSDB subset={subset} from {musdb_root}")
        ...
        for i, track in enumerate(tqdm(db, desc=f"Processing {subset}", unit="track")):
            if args.max_track is not None and i >= args.max_track:
                break
            ...
            stats = build_track_npz(
                track=track,out_path=out_path,
                sr=args.sr,win_sec=args.win_sec,hop_sec=args.hop_sec,n_fft=args.n_fft,
                mel_hop=args.mel_hop,
                n_mels=args.n_mels,ratio_thr_map=ratio_thr_map,min_mix_rms=args.min_mix_rms,
        ...
        print(f"Subset {subset} done.")
        print(f"  Saved tracks         : {saved}")
        print(f"  Skipped tracks       : {skipped}")
        print(f"  Total windows kept   : {tot_kept}")
